...
@section('title', 'رفع منتجات (كرتونة) - مطابقة الطلب #' . $order->id)
            ...
            <div class="box box-primary" style="border-radius: 15px; overflow: hidden; border: none; box-shadow: 0 10px 30px rgba(0,0,0,0.05);">
                <div id="full_page_content" style="padding: 20px;">
                    <h2 id="page_mode_title" style="margin-top: 0; margin-bottom: 25px; font-weight: bold; color: #333;">
                        <i class="fa fa-cubes text-primary"></i> رفع منتجات بالكرتونة (Standard Mode)
                    </h2>
                                        ...
                                        <table class="table table-bordered table-striped text-center" id="batch_table" style="margin-bottom: 0; font-size: 14px;">
                                            <thead style="background: #3c8dbc; color: white;">
                                                <tr>
                                                    ...
                                                    <th style="vertical-align: middle; display: none;" id="lbl_batch_gross_weight">وزن G.W</th>
                                                    <th style="vertical-align: middle;">الطول</th>
                                                    <th style="vertical-align: middle;">العرض</th>
                                                    <th style="vertical-align: middle;">الارتفاع</th>
                                                    <th style="vertical-align: middle; display: none;" id="lbl_batch_total_cbm">حجم الكرتونة</th>
                                                    <th style="vertical-align: middle;" id="lbl_batch_units">الوحدات/كرتونة</th>
...
<script>
            ...
            containers.forEach((c, i) => {
                ...
                widgets.eq(i).html(`
                    <div style="font-size: 11px;">
                        <div>الكراتين: <b>${c.f === 1 ? tc.toFixed(3) : Math.floor(tc).toLocaleString()}</b></div>
    ...
    $('#btnAddToList').on('click', function() {
        ...
        var p = {
            ...
            description: desc, images: JSON.parse(JSON.stringify(initialSelectedImagesData)), upload_mode: 'carton'
        };
    ...
    function renderBatchTable() {
        ...
        productsBatch.forEach(p => {
            tbody.append(`<tr>
                ...
                <td style="display:none">${p.total_weight}</td>
                <td>${p.carton_length}</td><td>${p.carton_width}</td><td>${p.carton_height}</td>
                <td style="display:none">${p.carton_volume_cbm}</td>
