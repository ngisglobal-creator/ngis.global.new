      ...
      <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <div class="avatar-upload" style="position: relative; display: inline-block;">
                <div class="avatar-preview" style="width: 150px; height: 150px; border-radius: 50%; border: 3px solid #f8f9fa; box-shadow: 0 0 10px rgba(0,0,0,0.1); overflow: hidden; background: #eee; margin-bottom: 10px;">
                  <img id="imagePreview" src="{{ asset('dist/img/user4-128x128.jpg') }}" style="width: 100%; height: 100%; object-fit: cover;">
                </div>
          ...
          <div class="col-md-9">
            <div class="row">
              <div class="col-md-4">
                <div class="form-group">
                  <label>الاسم</label>
                  <input type="text" name="name" class="form-control" value="{{ old('name') }}" required placeholder="أدخل اسم المستخدم">
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                  <label>البريد الإلكتروني</label>
                  <input type="email" name="email" class="form-control" value="{{ old('email') }}" required placeholder="example@domain.com">
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <select name="country_id" id="country_id" class="form-control select2-countries">
                    ...
                    @foreach($countries as $country)
                      <option value="{{ $country->id }}" data-flag="{{ asset('vendor/flag-icons/flags/4x3/' . $country->flag_code . '.svg') }}" {{ old('country_id') == $country->id ? 'selected' : '' }}>{{ $country->name_ar }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
              ...
              <div class="col-md-6" id="stars_group" style="display: none;">
                <div class="form-group">
                  ...
                  <div class="star-rating">
                    <input type="radio" id="star5" name="stars" value="5" {{ old('stars') == 5 ? 'checked' : '' }} />
                    <label for="star5" title="5 stars"><i class="fa fa-star"></i></label>
                    <input type="radio" id="star4" name="stars" value="4" {{ old('stars') == 4 ? 'checked' : '' }} />
                    <label for="star4" title="4 stars"><i class="fa fa-star"></i></label>
                    <input type="radio" id="star3" name="stars" value="3" {{ old('stars') == 3 ? 'checked' : '' }} />
                    <label for="star3" title="3 stars"><i class="fa fa-star"></i></label>
                    <input type="radio" id="star2" name="stars" value="2" {{ old('stars') == 2 ? 'checked' : '' }} />
                    <label for="star2" title="2 stars"><i class="fa fa-star"></i></label>
                    <input type="radio" id="star1" name="stars" value="1" {{ old('stars') == 1 ? 'checked' : '' }} />
        ...
        <div class="row" style="margin-top: 15px;">
          ...
          <div id="package_group" class="col-md-12" style="display: none;">
            <h4 class="page-header" style="margin-top: 5px; font-size: 16px; border-bottom: 1px solid #eee; padding-bottom: 5px;"><i class="fa fa-cubes text-primary"></i> اختر الباقة</h4>
            ...
            <div class="form-group">
              ...
              <select name="permissions[]" class="form-control select2" multiple="multiple" style="width:100%;">
                @foreach($permissions as $perm)
                  <option value="{{ $perm->name }}" {{ in_array($perm->name, old('permissions', [])) ? 'selected' : '' }}>{{ $perm->name }}</option>
                @endforeach
              </select>
            </div>
          </div>
        ...
        <div class="box-footer text-left" style="background: transparent; border-top: 1px solid #f4f4f4; padding: 20px 0 0 0;">
          <button type="submit" class="btn btn-primary btn-lg px-5"><i class="fa fa-save"></i> إنشاء المستخدم</button>
