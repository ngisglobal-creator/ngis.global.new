<header class="main-header">
  <a href="{{ url('regional/dashboard') }}" class="logo">
    <span class="logo-mini">
        <img src="{{ \App\Models\Setting::logoUrl() }}" style="height: 30px;">
    </span>
    <span class="logo-lg"><b>{{ \App\Models\Setting::get('site_name', __('dashboard.regional_office')) }}</b></span>
  </a>
    ...
    <div class="navbar-custom-menu">
      <ul class="nav navbar-nav">
        ...
        <li class="dropdown messages-menu">
          <a href="{{ route('regional.wallet') }}" class="dropdown-toggle" title="المحفظة" style="display: flex; align-items: center; gap: 10px; padding: 10px 15px;">
        ...
        <li class="dropdown user user-menu">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">
            ...
            <span class="hidden-xs">{{ Auth::user()->name ?? 'مكتب اقليم' }}</span>
          </a>
          <ul class="dropdown-menu">
            <li class="user-header">
              <img src="{{ asset('dist/img/user4-128x128.jpg') }}" class="user-image" alt="صورة المستخدم">
              <p>
                {{ Auth::user()->name ?? 'مكتب اقليم' }}
                <small>مكتب اقليم</small>
              </p>
            </li>
